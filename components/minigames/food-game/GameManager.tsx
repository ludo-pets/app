...
export const useGameManager = (): GameManagerType => {
    ...
    const { foods, FOOD_TYPES, createFood, setFoods, updateCurrentLives } = useFoods({ config });
    ...
    const scoreThresholds = useRef([20, 40, 70, 100, 150, 200, 300, 400, 500, 600]);
    ...
    const increaseDifficulty = () => {
        setDifficulty((prev) => {
            ...
            const speedMultiplier = Math.pow(config.DIFFICULTY_SPEED_MULTIPLIER, newDifficulty);
    ...
    const spawnFood = () => {
        ...
        const foodMultiplier = 1 + (difficulty * config.DIFFICULTY_FOOD_MULTIPLIER);
        ...
        if (foods.length === 0 || foods.length < maxFoods) {
            ...
            if (difficulty > 3) {
                ...
                if (Math.random() * 100 < extraFoodChance && foods.length < maxFoods) {
                    setTimeout(() => {
                        if (gameStarted && !gameOver) {
                            createFood(createItemParam, score);
                        }
     ...
     */
    const checkCollisions = useCallback((callback: (type: string) => void) => {
    ...
    }, [characterPosition, foods, config, setFoods, setLifes, setScore, increaseDifficulty]);
