 ...
 */
export const scheduleNeedNotification = async (needType: NeedType, hoursFromNow?: number) => {
  if (Platform.OS === 'android' && !Device.isDevice) {
    console.log(`[Notificação Agendada Simulada]: ${needsConfig[needType].notificationTitle}`);
    return;
  }
  ...
  await Notifications.scheduleNotificationAsync({
    content: {
      title: needsConfig[needType].notificationTitle,
      body: needsConfig[needType].notificationBody,
  ...
  console.log(`Notificação agendada para ${needType} em ${seconds / 3600} horas`);
};
